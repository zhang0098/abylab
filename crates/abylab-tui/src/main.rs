...
fn main() -> Result<()> {
    ...
    restore_terminal();
    // Back on the normal screen: the session id and the two ways back. The app
    // owns the wording (locale + the session it ended on), the shell writes it,
    // so it survives the alternate screen (see `App::exit_notice`).
