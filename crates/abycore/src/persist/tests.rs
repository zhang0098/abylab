...
fn checkpoints_rewrite_to_header_title_and_latest_snapshot() {
    ...
    for seq in 0..10u64 {
        ...
        store.append_checkpoint(&mut writer, seq, &snapshot).unwrap();
    }
...
fn deltas_append_between_anchors_and_fold_to_the_latest_state() {
    ...
    assert!(lines.iter().skip(1).filter(|l| l.contains("\"type\":\"snapshot\"")).count() == 1);
    assert_eq!(lines.iter().filter(|l| l.contains("\"type\":\"delta\"")).count(), 9);
...
fn in_place_record_updates_survive_the_delta_fold() {
    ...
    let text = fs::read_to_string(store.log_path("s").unwrap()).unwrap();
    assert!(text.contains("\"type\":\"delta\""), "the second checkpoint is a delta");
    let (_, loaded) = store.load("s").unwrap();
    assert_eq!(
        loaded.requests[0].usage.as_ref().and_then(|u| u.input_tokens),
        Some(10),
...
fn oversized_delta_demotes_to_an_anchor() {
    ...
    snapshot.items = vec![
        Item::user("tiny"),
        Item::user("huge ".repeat(200_000)),
    ];
    store.append_checkpoint(&mut writer, 1, &snapshot).unwrap();
    drop(writer);
    let text = fs::read_to_string(store.log_path("s").unwrap()).unwrap();
    assert!(!text.contains("\"type\":\"delta\""), "a giant item demotes to an anchor");
