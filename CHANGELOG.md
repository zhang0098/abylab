...
- 排队消息现在能扛住重启：队列是唯一一样驱动不拥有、崩溃会丢的东西，所以每次变动都
  写 `$ABYLAB_HOME/queued/<会话>.json`（每个会话一个文件，图片负载一起存，排空即删）。启动或 `/resume` 到
  那个会话会把条目画回时间线并**先按住**——它们原本排在一个已经不存在的回合后面，
  不会替你花掉；下一次真实回合结束（或你显式 enter／ctrl+enter）时按 FIFO 放出。
  文件缺失或损坏就当成空队列，行为和 `settings.json` 一致。
