...
插话的三段状态在时间线上是看得见的：按下 ctrl+enter 时气泡先带上 `steering` 标记
（待插话），SDK 在边界收下它的那次 drain 会触发一次 checkpoint，驱动据此回报
`SteerAdmitted`，标记随即消失；被拒绝或错过窗口的则回到 `queued`

排队仍然是客户端行为：消息留在 TUI 的 FIFO 里（带排队底色、`⌥↑` 可编辑、插话或
删除），回合结束、运行状态转为空闲时按顺序发出。`⌥↑` 列表里 enter 编辑、
ctrl+enter 把高亮那条立即插话、ctrl+d 删除，对应 harness 的 QueueDock 行操作。

正因为队列在客户端而不在驱动里，它是唯一一样进程崩掉就会丢的东西：会话快照是持久的，
排在它后面的未发消息不是。所以每次队列变动都会写一遍
`$ABYLAB_HOME/queued/<session>.json`（每个会话一个文件，含图片负载；排空即删文件），启动或 `/resume` 到该会话时把条目原样画回时间线
（仍是排队底色），并**先按住**：这些消息原本排在一个已经不存在的回合后面，替用户
花掉它们不是 app 该做的事。下一次真实回合结束（或用户显式 enter／ctrl+enter）时
按 FIFO 放出。文件缺失、损坏或被删都只是空队列，和 `settings.json` 一个待遇。
