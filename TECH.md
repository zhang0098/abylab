...
比 harness 更严的地方：abylab 自己加了两道兜底，都是「别把回合吊死」，不是「别干太久」。

`ABY_TURN_TIMEOUT`（默认 600 秒）是一个轮段**毫无进展**的上限——没有字节到达、没有
请求或工具收尾、没有提交的步骤——而不是整段时长的上限。持续在干活的回合想跑多久就跑
多久：每读到一个流块、每次提交工具结果都会把计时归零，等待中的 await 每次醒来重读
一遍这个间隔，所以长回答、长工具序列都不会被它截断。真正卡死网络的是传输层自己的
超时（连接 15 秒、首字节 120 秒、流空闲 60 秒），所以这道窗口只在「整个回合不动了」
时才响；停滞的请求也不会在传输层反复重试，直接交给驱动去续跑。

`ABY_TOOL_TIMEOUT`（默认 60 秒）是**自己没声明预算的工具调用**的兜底；工具自己声明的
预算说了算——harness 也是按工具读它自己的 `timeoutMs`，而不是给所有工具套一个数。
`bash` 带上 `timeoutMs` 就以它为准（`bash_max_timeout` 封顶，默认 600 秒），前台委派
子代理、`wait_agent` 的等待按子回合的运行窗口算。工具调用到点会先被请求停止、再用它
自己的 `cleanup_grace` 收尾，结果作为一条工具错误交回模型（写明被中止、结果未核实），
回合继续往下走——harness 的 `dsh-tool-call-timeout-policy` 同样是回一条错误结果，而不是
让回合失败。

轮段真的停滞、或请求／工具各自到点时，会等待 1 秒再在同一回合内自动续跑；它与请求／
工具预算耗尽、临时网络错误共用 `ABY_AUTO_CONTINUE` 次数（默认 3，设为 0 则不自动续跑）。
已完成的工具结果会保留；执行中被截断的工具会标记为“结果未核实，重复执行前先检查”，
不会自动重放。次数用尽才报错，错误会列出当前环境变量值，继续输入仍可续跑；Esc 可以
中断执行和重试等待。（这些预算和时限是 abylab 自己的兜底，不是 DeepSeek 的限制 ——
用量照常计费。）
