...
技能在驱动启动时扫一次，整场会话共用这份快照：`/` 菜单的技能行、`/skill` 的清单、
`skill` 工具和 `/名字` 的注入读的都是它。加了新文件要重开会话才生效。
...
- 内置命令的优先级不变：技能取到同名会被内置命令挡住，同样 `//`、`/usr/bin`
  这类带斜杠的行不会被当成技能。要跑一个和内置同名的技能，用 `/skill <名字> [参数]`，
  它把名字解析出来后照样发 `/<名字> [参数]`。
